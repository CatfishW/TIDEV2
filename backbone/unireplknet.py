...
def load_chekpoint(model, path):
    ...
    temp = {k: v for k, v in checkpoint.items() if k in model_dict and v.size() == model_dict[k].size()}
    model_dict.update(temp)
    model.load_state_dict(model_dict)

def initialize_with_pretrained(model, path):
    load_chekpoint(model, path)
...
@register_model
def unireplknet_f(in_1k_pretrained=False, path=None, **kwargs):
    model = UniRepLKNet(depths=UniRepLKNet_A_F_P_depths, dims=(48, 96, 192, 384), **kwargs)
    initialize_with_pretrained(model,path)
    return model
...
@register_model
def unireplknet_t(in_1k_pretrained=False,path=None, **kwargs):
