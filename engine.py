...
def train_one_epoch(model: torch.nn.Module, criterion: torch.nn.Module,
    ...
    for sample_query, sample_support, targets,text_features in metric_logger.log_every(data_loader, print_freq, header):
        sample_query, sample_support = sample_query.to(device), sample_support.to(device)
        targets = [{k: v.to(device) for k, v in t.items()} for t in targets]
        text_features = text_features.to(device) if text_features is not None else None

        if scaler is not None:
            with torch.autocast(device_type=str(device), cache_enabled=True):
                outputs = model(sample_query,sample_support, targets)
            ...
            outputs = model(sample_query,sample_support, targets,text_features)
            loss_dict = criterion(outputs, targets)
